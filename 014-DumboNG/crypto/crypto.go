...
func (s *SignatureShare) Verify(Digest) bool {
	return len(s.PartialSig) > 0
}

func VerifyTsPartial(pub *share.PubPoly, digest Digest, sig SignatureShare) error {
	suite := bn256.NewSuite()
	return tbls.Verify(suite, pub, digest[:], sig.PartialSig)
}
