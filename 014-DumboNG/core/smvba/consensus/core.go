...
func (c *Core) PbBroadcastBlock(preHash crypto.Digest) error {
	...
	block := c.generatorBlock(height, preHash)
	bm, _ := NewBlockMessage(c.Name, block, height, c.SigService)
...
func (c *Core) handleBlockMessage(bm *BlockMessage) error {
	...
	c.BlockHashMap[bm.Author][bm.Height] = blockHash
	c.blocklock.Unlock()
	...
	if err := c.PbSendVote(bm.Author, bm.Height, blockHash); err != nil {
		return err
	}
	c.TrytoStartMVBA()
	return nil
...
func (c *Core) handleFinvote(fv *FinVote) error {
	...
	if flag, blockHash, err := c.Aggreator.AddFinVote(fv); err != nil {
		return err
	} else if flag == ACTION_YES {
		return c.advanceNextRound(fv.Epoch, fv.Round, flag, blockHash)
	} else if flag == ACTION_NO {
		return c.advanceNextRound(fv.Epoch, fv.Round, flag, crypto.Digest{})
	} else if flag == ACTION_COMMIT {
		halt, _ := NewHalt(c.Name, fv.Leader, blockHash, fv.Epoch, fv.Round, c.SigService)
		c.Transimtor.Send(c.Name, core.NONE, halt)
		c.Transimtor.RecvChannel() <- halt
	}
...
func (c *Core) handleHalt(h *Halt) error {
	...
	if ready, err := c.Aggreator.AddHaltVote(h); err != nil {
		return err
	} else if !ready {
		return nil
	}
...
func (c *Core) Run() {
	...
	for {
		...
		case BlockMessageType:
			err = c.handleBlockMessage(msg.(*BlockMessage))
		case VoteforBlockType:
			err = c.handleVoteForBlock(msg.(*VoteforBlock))
		}
		if err != nil {
			logger.Warn.Println(err)
		}
